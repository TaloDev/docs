...
export const toc = [
  ...
  {
    value: 'Identifying players using Talo Authentication',
    id: 'identifying-players-using-talo-authentication',
    level: 2,
  },
...
:::info Session token lifetimes
By default session tokens are long-lived. You can opt in to short-lived (15-minute) session tokens paired with a 30-day refresh token using the `withRefresh` parameter. See [Session token lifetimes](/docs/http/authentication#session-token-lifetimes) for the full details.
:::

...
:::info
This signal only fires for channel props (set during `create()` and `update()`). For channel **storage** prop errors, use `channel_storage_props_failed_to_set` instead.
:::
...
Sometimes, setting storage props can fail. This usually happens when you set a prop key with a size over 128 characters or a prop value with a size over 512 characters. The `Talo.channels.channel_storage_props_failed_to_set` signal lets you listen for these errors:

...
If a player doesn't have a prop matching the configured key, their display name falls back to their alias `identifier`.

The display name is resolved by Talo on each identification, so it stays up to date as the player's props change.

...
You also need to know the target player alias' ID. You can get this through methods such as [player presence updates](/docs/unity/player-presence), or by searching for players using [player search](/docs/unity/player-props#searching-for-players).
...
This method allows you to filter by unconfirmed subscription requests. You can iterate over the results and confirm each subscription individually using their ID:
...
This method accepts the same filter options as `GetSubscriptions()`:
...
void OnMessageReceived(PlayerAlias playerAlias, string message)
{
    ...
    try
    {
        var data = JsonUtility.FromJson<EventData>(message);

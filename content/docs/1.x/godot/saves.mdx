...
Both functions are awaitable: `update_current_save()` returns a `SaveUpdateResult` (`success`, `save`), while `update_save()` returns the updated `TaloGameSave`.

The `Talo.saves.save_updated(success: bool, save: TaloGameSave)` signal fires after every save update settles (`save` is `null` on failure), and `Talo.saves.flush_updates()` immediately flushes any pending or in-flight update (returning a `FlushResult`). Pending updates are also flushed automatically when the game quits.

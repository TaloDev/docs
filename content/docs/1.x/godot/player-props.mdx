...
<Callout type="info">
  The result's `success` property only reflects whether the network request succeeded - it will be set to `true` even when props are rejected.

  To find out whether your props were actually accepted, check `rejected_props`.
</Callout>

The prop mutators return an awaitable `PlayerUpdateResult`. Rapid mutations are coalesced into a single PATCH and every caller in the same batch settles together - you can read the rejected props off the result:

```gdscript
var result: PlayersAPI.PlayerUpdateResult = await Talo.current_player.set_prop("level", "5")

for prop in result.rejected_props:
	# prop.key: the key of the rejected prop
	# prop.code: the rejection reason code (e.g. TaloRejectedProp.ErrorCode.PROP_VALUE_TOO_LONG)
...
The `TaloRejectedProp` class exposes a `code` property of type `TaloRejectedProp.ErrorCode`:
...
for rejected_prop in result.rejected_props:
	match rejected_prop.code:
		TaloRejectedProp.ErrorCode.PROP_KEY_TOO_LONG:
			push_warning("Key too long: %s" % rejected_prop.key)
		TaloRejectedProp.ErrorCode.PROP_VALUE_TOO_LONG:
			push_warning("Value too long for key: %s" % rejected_prop.key)
		TaloRejectedProp.ErrorCode.PROP_CONTAINS_PROFANITY:

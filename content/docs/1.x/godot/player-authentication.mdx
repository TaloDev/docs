...
The `login()` function returns a `PlayerAuthLoginResult` with a `success` flag and a `verification_required` flag. Check `verification_required` to see if the player must verify their details before their session is created:
...
If the email is already in use, the error code will be `TaloPlayerAuthError.ErrorCode.EMAIL_TAKEN`.
...
if (await Talo.player_auth.forgot_password(email.text)).success:

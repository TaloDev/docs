...
`send()` returns a `FeedbackSendResult` with a `success` flag and a `rejected_props` array:

```gdscript
var result := await Talo.feedback.send(category_internal_name, comment, props)

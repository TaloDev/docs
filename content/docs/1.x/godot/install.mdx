...
<Callout type='warn'>
  **Download Talo only from the sources listed on this page.**

  Talo is completely free to download - if you see it for sale anywhere else (e.g. priced listings on other stores), that listing is not affiliated with Talo.
</Callout>

### Using the Asset Store

The Godot Asset Store is the recommended way to download Godot plugins. Open the `Asset Store` tab in Godot and search for "Talo Game Services". Click into the asset and click `Download`. Here's a [direct link](https://store.godotengine.org/asset/talo/talo-game-services) to the asset.

![Talo in the Godot Asset Store tab](/img/godot-install-asset-store.png)

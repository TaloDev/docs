...
print(result.channel.name) # channel name
print(result.channel.auto_cleanup) # true
print(result.channel.props) # [{ "prop_key": "prop_value" }]
```
...
options.name = "new channel name"
options.new_owner_alias_id = new_owner_alias.id
options.props = {
	team = "red"
}
options.auto_cleanup = Talo.channels.UpdateChannelOptions.AutoCleanup.ENABLED
options.private = Talo.channels.UpdateChannelOptions.Privacy.DEFAULT
options.temporary_membership = Talo.channels.UpdateChannelOptions.TemporaryMembership.ENABLED

var result := await Talo.channels.update(channel.id, options)
if not result.success:
	return
```

The `auto_cleanup` and `temporary_membership` options are tri-state toggles (`DEFAULT` / `ENABLED` / `DISABLED`), and `private` is `DEFAULT` / `PRIVATE` / `PUBLIC`. A `DEFAULT` value leaves the field unchanged - set it explicitly to change it.
...
if not result.success:
	return
print(result.channel.private) # true
```
...
if not result.success:
	return
await Talo.channels.invite(result.channel.id, invitee_player_alias.id)
```
...
`create()` and `update()` return a `ChannelUpsertResult` with a `success` flag and a `rejected_props` array:
...
The `Talo.channels.channel_storage_props_failed_to_set` signal lets you listen for these errors. It passes an array of `TaloRejectedProp` objects:

```gdscript
func _ready() -> void:
	Talo.channels.channel_storage_props_failed_to_set.connect(
		func (channel: TaloChannel, failed_props: Array[TaloRejectedProp]):

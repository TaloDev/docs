...
You can identify a player using `Talo.players.identify()`, which returns a `TaloPlayerAlias`. The code sample below shows you how you could identify a player using a UI element (this example is also available in the Playground scene):
...
After a successful identification, the `Talo.players.identified` signal will emit, returning the identified player's alias. This allows you to, for example, put the player's identifier on a label:
...
If identification fails, the `Talo.players.identification_failed` signal is emitted. It passes a `TaloIdentifyError` with a `code` enum (`UNKNOWN_ERROR`, `IDENTIFIER_PROFANITY`, `IDENTIFIER_TAKEN`):
...
func _on_identification_failed(error: TaloIdentifyError) -> void:

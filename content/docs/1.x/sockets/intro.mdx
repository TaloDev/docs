...
1. You must provide a [socket ticket](/1.x/http/socket-ticket-api) when opening a socket connection. This identifies your game and the key's scopes are used to determine whether a client is eligible to receive specific responses.
...
Your ticket should be a valid socket ticket, i.e. it was created within the last 5 minutes. Visit the [API docs](/1.x/http/socket-ticket-api) for more information on how to obtain a socket ticket.
...
For a more detailed look into the available error codes, visit the [common errors](/1.x/sockets/common-errors) page.

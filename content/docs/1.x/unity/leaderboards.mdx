...
{
	var result = await Talo.Leaderboards.AddEntry(internalName, score);
	Debug.Log(result.Entry.position);
}
```

This function returns an `AddEntryResult` with the entry and whether the entry was updated.
...
`AddEntry()` returns an `AddEntryResult` with a `Success` flag and a `RejectedProps` array:

```csharp
var result = await Talo.Leaderboards.AddEntry(leaderboardName, score, ("team", team));
...
You can use `Talo.Leaderboards.GetCachedEntries()` in the same way as `GetEntries()` above. Every entry fetched previously using `GetEntries()` will exist in the cache.

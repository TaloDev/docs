...
<Callout type='warn'>
  ...
  avoid setting or deleting props in `Update()` functions.
</Callout>
...
<Callout type="info">
  ...
  To find out whether your props were actually accepted, check `RejectedProps`.
</Callout>

The prop mutators return an awaitable `Task<PlayerUpdateResult>`. Rapid mutations are coalesced into a single PATCH and every caller in the same debounce window settles together - you can read the rejected props off the result:
...
foreach (var rejectedProp in result.RejectedProps)
{

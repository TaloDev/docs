...
You can identify a player using `Talo.Players.Identify()`, which returns a `PlayerAlias`. The code sample below shows you how you could identify a player using a UI element (this example is also available in the Playground):
...
Visit the [Player authentication docs](/1.x/unity/player-authentication) to learn more about identifying players with authentication enabled.

</Callout>

### The "OnIdentified" event

After a successful identification, the `Talo.Players.OnIdentified()` event will invoke, returning the identified player's alias. This allows you to, for example, immediately fetch that player's saves:

```csharp
Talo.Players.OnIdentified += async (alias) =>
{
...
If identification fails, the `Talo.Players.OnIdentificationFailed` event is invoked. It passes an `IdentifyException` with an `ErrorCode` enum (`UNKNOWN_ERROR`, `IDENTIFIER_PROFANITY`, `IDENTIFIER_TAKEN`).
...
public class IdentifyPlayer : MonoBehaviour
{
	...
	{
		Debug.Log($"Player identifier: {alias.player.id}");
	}
}
```
...
public class WelcomeLabel : MonoBehaviour
{
	private void Start()
	{
		Talo.Players.OnIdentified += (alias) =>
		{
...
![The display name prop key setting in the dashboard](/img/display-name-prop-key-setting.png)

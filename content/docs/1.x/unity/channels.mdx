...
Channel[] channels = res.channels;
int count = res.count;
bool isLastPage = res.isLastPage;
```
...
var result = await Talo.Channels.Create(new CreateChannelOptions() { name = channelName, autoCleanup = true, props = props });
...
You can also update the props of the channel: keys will be overridden with new values and keys with a value of `null` will be deleted.

`Update()` takes an `UpdateChannelOptions` object:
...
The `autoCleanup`, `isPrivate` and `temporaryMembership` options are nullable (`bool?`) so you can leave a field unchanged (`null`) or explicitly set it to `true`/`false`. `name` (empty string) and `newOwnerAliasId` (`-1`) work the same way.
...
```csharp
var result = await Talo.Channels.Create(new CreateChannelOptions() { name = "channel name", isPrivate = true });
if (!result.Success) return;
await Talo.Channels.Invite(result.Channel.id, inviteePlayerAlias.id);
```
...
```csharp
var result = await Talo.Channels.Create(new CreateChannelOptions() { name = "channel name", temporaryMembership = true });
if (!result.Success) return;
```
...
`Create()` and `Update()` return a `ChannelUpsertResult` with a `Success` flag and a `RejectedProps` array:

```csharp
var result = await Talo.Channels.Create(createOptions);
...
For **channel storage** prop errors, use the `OnChannelStoragePropsFailedToSet` event instead.
...
`RejectedProp` includes a `message` with a human-readable description of the error, alongside the `error` code. See [Players and props - Prop rejection reasons](/1.x/unity/player-props#prop-rejection-reasons) for the full list of rejection reason codes.
...
If any of the keys you pass are [prop arrays](/1.x/unity/channels#getting-storage-prop-arrays), append `[]` to the key to fetch every item stored under it. Scalar and array keys can be mixed in a single call:

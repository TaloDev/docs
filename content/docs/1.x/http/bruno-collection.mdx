...
The collection's **Game API** folder contains requests for the gameplay-orientated endpoints used by your game, organised into subfolders like **Players**, **Stats**, **Leaderboards** and **Player auth**.
...
| `apiKey`  | A game API key from the [Talo dashboard](https://dashboard.trytalo.com/api-keys). The **Game API** folder uses this. |
...
To get these, run the [Identify](/1.x/http/player-api) endpoint first (found in the **Game API** > **Players** folder of the collection). This will automatically set the variables for both of those headers.

### Talo player authentication

If your game uses [Talo's player authentication](/1.x/http/player-auth-api), you'll also need the `x-talo-session` header.

Run the **register** and **login** requests (from the **Game API** > **Player auth** folder). This will set the `x-talo-session` variable and refresh token variable (for refreshing the short-lived session token).

See [Player authentication](/1.x/http/authentication#player-authentication) for the full flow.

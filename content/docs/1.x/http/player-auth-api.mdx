...
Once you have registered a player, you can identify them using the [identify](/1.x/http/player-api) endpoint. You will need to ensure that:
...
<Callout type='info' title='Session token lifetimes'>
  ...
  lifetimes](/1.x/http/authentication#session-token-lifetimes) for the full details.
</Callout>

...
A valid socket ticket is required when opening a socket connection. Learn more about how to obtain a socket ticket and connect to the socket server [here](/pre-1.0/sockets/intro#connecting-to-the-socket-server).
...
If your game has [request verification](/pre-1.0/sockets/request-verification) enabled, all socket messages sent after identifying a player must include a valid signature. If the signature is missing, malformed or fails Talo's validation, you will receive this error.

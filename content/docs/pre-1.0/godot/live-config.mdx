...
```gdscript
var halloween_event_enabled = Talo.live_config.get_prop("halloween_event_enabled", "false") == "true"
```

`live_config.get_prop()` will return the value of the property as a string, so you will need to convert it to the correct type - for booleans, compare the string to `"true"` rather than using `bool()`, which treats any non-empty string (including `"false"`) as `true`. This function also has a second argument which is the default value to return if the property is not found.
...
The `live_config_updated` signal is emitted (via the [Talo Socket](/pre-1.0/godot/socket)) whenever the live config is updated.

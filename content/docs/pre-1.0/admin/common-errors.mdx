...
Admin API keys are prefixed with `ta_` and are created inside the [dashboard](https://dashboard.trytalo.com/api-keys). Unlike the gameplay [HTTP API](/docs/pre-1.0/http/authentication), admin API keys are scoped to a single game rather than authenticating on behalf of a player.
...
Learn more in the [dev data guide](/docs/pre-1.0/admin/dev-data).

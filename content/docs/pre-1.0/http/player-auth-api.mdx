...
Once you have registered a player, you can identify them using the [identify](/pre-1.0/http/player-api) endpoint. You will need to ensure that:
...
<Callout type='info' title='Session token lifetimes'>
  ...
  lifetimes](/pre-1.0/http/authentication#session-token-lifetimes) for the full details.
</Callout>

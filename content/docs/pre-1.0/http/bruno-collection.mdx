...
To get these, run the [Identify](/pre-1.0/http/player-api) endpoint first (found in the **Players** folder of the collection). This will automatically set the variables for both of those headers.

### Talo player authentication

If your game uses [Talo's player authentication](/pre-1.0/http/player-auth-api), you'll also need the `x-talo-session` header.
...
See [Player authentication](/pre-1.0/http/authentication#player-authentication) for the full flow.

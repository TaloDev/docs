...
You can learn more about how the socket works [here](/docs/unity/sockets/intro).
...
The `OnMessageReceived` event provides you with a [response](/docs/unity/sockets/responses) and payload. For example, if you were building a chat system, you would connect a function similar to the one below to listen for new chat messages:
...
Generally, sending messages is handled by functions in services like `Talo.Channels.SendMessage()` where the correct data is prepared and sent for you. You can also use the `Talo.Socket.Send()` function to send your own [requests](/docs/unity/sockets/requests):
...
The socket server can disconnect for a number of reasons such as the player going offline or being [rate limited](/docs/unity/sockets/common-errors#rate-limit-exceeded). The socket will invoke a `OnConnectionClosed` event with a status code and reason.
